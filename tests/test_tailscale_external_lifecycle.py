...
def safe_launcher_diagnostic(output: bytes) -> str:
    ...
    markers = (
        ...
        (b"managed-state reprint: active operator-owned Serve control identity or local release is unavailable", "external_reprint_control_status_mismatch"),
        (b"managed-state reprint: open root:", "external_reprint_root_unavailable"),
        (b"managed-state reprint: a staged journal is present", "external_reprint_journal_present"),
        (b"managed-state reprint: inspect journal:", "external_reprint_journal_inspection_failed"),
        (b"managed-state reprint: acquire transaction:", "external_reprint_transaction_unavailable"),
        (b"managed-state reprint: begin journal: managedstate: foreign managed state", "external_reprint_journal_foreign_state"),
        (b"managed-state reprint: begin journal: managedstate: invalid managed record", "external_reprint_journal_invalid_record"),
        (b"managed-state reprint: begin journal: managedstate: filesystem operation failed", "external_reprint_journal_filesystem_failure"),
        (b"managed-state reprint: begin journal: managedstate: unknown managed authority", "external_reprint_journal_authority_unavailable"),
        (b"managed-state reprint: begin journal:", "external_reprint_journal_begin_failed"),
        (b"managed-state reprint: pairing control rejected the bootstrap invitation", "external_arm_rejected_without_code"),
        (b"managed-state reprint: the bootstrap invitation state is uncertain", "external_arm_ack_uncertain"),
        (b"operator-owned relay control endpoint is unavailable", "external_control_endpoint_unavailable"),
        (b"Private control identity or local release did not match", "external_private_control_identity_mismatch"),
        (b"HTTPS health belongs to a different instance, run, transport, or origin", "external_https_health_identity_mismatch"),
        (b"External invitation arming was busy or definitely refused", "external_invitation_arm_refused"),
        (b"External invitation arming was refused", "external_invitation_arm_refused"),
        (b"Durable invitation acknowledgement was incomplete", "external_invitation_ack_incomplete"),
...
def assert_forced_shutdown_recovery(
    root: pathlib.Path,
    env: dict[str, str],
...
""",
        ...
        try:
            ...
            PHASE = "independent_sigkill_recovery_evidence"
            independently_killed, _ = start_launcher(root, env, active)
            ready_session = dict(
                line.split("=", 1)
                for line in (config_dir / "tailscale-external-session.env").read_text(encoding="utf-8").splitlines()
                if "=" in line
            )
            relay_pid = int(ready_session["HERDR_RELAY_PID"])
            os.kill(relay_pid, signal.SIGKILL)
            try:
                killed_output, _ = independently_killed.communicate(timeout=15)
            except subprocess.TimeoutExpired:
                fail("independently killed relay did not finish launcher cleanup")
            active.remove(independently_killed)
            session_path, control_path, recovery_log = assert_forced_shutdown_recovery(
                root, env, config_dir, relay_port, origin, sentinel,
                independently_killed, killed_output, 137, relay_pid,
            )
            ...
            PHASE = "launcher_sigkill_recovery_evidence"
            ...
            forced_pid = int(forced_session["HERDR_RELAY_PID"])
            ...
            assert_forced_shutdown_recovery(
                root, env, config_dir, relay_port, origin, sentinel,
                forced, forced_output, 130, forced_pid,
            )
            record("launcher_sigkill_preserves_recovery_record_socket_and_log")
        finally:
