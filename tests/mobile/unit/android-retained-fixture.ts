...
export function retainedFixture(handles: string[], selectedHandle: string, installed: string, startedAt: number): RetainedInspectionResult {
  ...
  const observations = handles.map((targetId, index) => ({ targetId, document: {
    href: 'https://fixture.test/', origin: 'https://fixture.test',
    timeOrigin: startedAt - 1, backendNodeId: index + 1,
  } }));
  const selectedDocument = {
    ...observations.find(o => o.targetId === selectedHandle)!.document,
    standalone: selectedHandle === installed,
    provider: selectedHandle === installed ? 'android-standalone' as const : 'browser' as const,
  };
  ...
  return structuredClone({
    kind: 'bounded-nonactivating-core-observation', phase: selectedHandle === installed ? 'installed-selected' : 'initial-browser-selected',
