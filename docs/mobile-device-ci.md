...
A manual workflow dispatch requires `artifact_run_id`, the completed successful same-repository `check` push on `main` that contains `release-bundles`; use `provenance_mode: external` and provide `source_commit` when qualifying a specific revision. The reusable workflow validates repository, workflow, push event, `main` branch, conclusion, source SHA, exact artifact name, and expiration before provisioning devices. The check and release reusable calls use `provenance_mode: internal`: they consume the caller's exact artifact without requiring the still-running parent to have a final conclusion. The gate downloads every evidence artifact and validates each expected platform/baseline/suite/source/hash/native-provider result, including one synthetic current-code result per platform. The release workflow invokes the same file with the exact artifact produced by its `build` job, and `publish` depends on the mobile result. Preparation validates the Linux candidate for Android and the Darwin arm64 candidate for iOS against the archive checksum, release manifest, web descriptor, hashed assets, and web-tree hash. It also runs a real Chromium/Go-fixture cache-recovery check with normal browser caching: the baseline loads, the target activates, a target-only missing stylesheet response is consumed, the failed phone plan is observed, and the candidate is reached through the shipped recovery button after the fault is repaired.
