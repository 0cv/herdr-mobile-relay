...
device state but cannot send input, answer approvals, upload, or change relay
...
expanded to a private host path only immediately before an authorized prompt is
dispatched. Reused or replaced panes cannot inherit an old attachment.
