...
# Herdr gateway deployment settings for @HOST@, generated by
# relay/gateway-deploy.sh. docker compose reads this file automatically from the
# directory it runs in. Every value below is also the compose default, so
...
them — so this box needs no trust, only bandwidth.
...
UDP 3478 is worth opening even though nothing user-visible dials it. A phone and
a computer that both sit behind NAT do not know the address the internet sees
them at, so neither can tell the other where to send packets. Each one asks this
gateway, which answers with the address it already observes on the connection,
and the pair then meets directly — that is what lets a phone on a cellular
network reach a home computer without a forwarded port, instead of paying the
latency and bandwidth of a relayed path through this box. It is published
directly on the host rather than proxied because raw UDP cannot travel through
Caddy's TLS.
