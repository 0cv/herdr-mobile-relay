...
> [!IMPORTANT]
> Native Windows is not supported. You need Herdr 0.7.5 or newer, Git, and `curl`.
...
You do not need Python, `uv`, Node.js, npm, Go, a separately hosted web app, or `sudo` for this path. The plugin build downloads and verifies the exact pre-built relay release.
...
After the stable service is running with Herdr 0.7.5 or newer, Settings can check and install later versioned relay updates one computer at a time. Each update checksum-verifies and atomically activates one complete executable-and-web release; a failed post-restart verification reactivates the previous complete release. Source checkouts remain explicitly checkout-managed. A separately hosted phone app remains a separate deployment.
